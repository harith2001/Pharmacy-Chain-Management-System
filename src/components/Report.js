...
const Report = () => {
  ...
  const total_stock = getstockdata.reduce(
    (total, element) => total + element.Medicine_NO,
    0
  );
  const total_medicine = getstockdata.length;
  const low_stock = getstockdata.filter(
    (element) => element.Medicine_NO < low
  ).length;
  ...
  return (
    <div className="report">
      <div ref={componentPDF} style={{ width: "100%" }}>
        ...
        <h5 className="total">
          {" "}
          Total Stock Count of Every Medicine In the Inventory :- {total_stock}
        </h5>
        <h5 className="total">
          {" "}
          Total Medicines In the Inventory :- {total_medicine}
        </h5>
        <h5 className="total">
          {" "}
          Total Low Stock Medicines In the Inventory :- {low_stock}{" "}
        </h5>
      </div>
      <div>
        <br></br>
      ...
      <div>
        ...
        <NavLink to={`/`}>
          {" "}
          <button className="PDF">Home</button>
        </NavLink>
      </div>
    </div>
  );
